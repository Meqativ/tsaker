...
function showTasks(first){
	...
	console.log((doneTasks===tasks.length) ? `🎉 # You've completed all of your tasks! You can clear them by doing the clear command` : `${"i".rgb({fg:[100,100,100]})} # You've got ${tasks.length} task${tasks.length===1?"":"s"}, ${doneTasks} of which ${doneTasks===1?"is":"are"} completed`)

	let i = 0
	for (let task of tasks) {
		i++
	console.log(`${task.done ? "~".rgb({fg:[10,200,10]}) : "•".rgb({fg:[100,100,100]})} | ${i} | ${task.goal}`)	
	}
}

function removeTaskJob(args){
	let index = parseInt(args[1])
	if (Number.isNaN(index)) return console.log(`${"×".rgb({fg:[200,10,10]})} | Invalid task index number`)
	index -= 1
	const removedTask = tasks.splice(index, 1)[0]
	console.log(`${"✓".rgb({fg:[10,200,10]})} | Removed task with goal: "${removedTask.goal}"`)
...
function toggleTaskJob(args){
	...
	console.log(`${"✓".rgb({fg:[10,200,10]})} | Toggled the task with goal: "${tasks[index].goal}", to ${tasks[index].done ? "completed" : "pending"}`)
}
function addTaskJob(args){
	if (!args[1] && args[1] !== "") return console.log(`${"×".rgb({fg:[200,10,10]})} | No goal specified`)
	tasks.push({goal: args.slice(1, args.length).join(" "), done: false})
	console.log(`${"✓".rgb({fg:[10,200,10]})} | Added`)
}
function moveTaskJob(args){
	if (args.length === 1) return console.log(`${"×".rgb({fg:[200,10,10]})} | indexOne and indexTwo is not specified. See in the help command`)
	let iOne = parseInt(args[1])
	if (Number.isNaN(iOne)) return console.log(`${"×".rgb({fg:[200,10,10]})} | Invalid indexOne number`)
	let iTwo = parseInt(args[2])
	if (Number.isNaN(iTwo)) return console.log(`${"×".rgb({fg:[200,10,10]})} | Invalid indexTwo number`)
		iOne--
		iTwo--
	tasks.swap(iOne, iTwo)
	return console.log(`${"✓".rgb({fg:[10,200,10]})} | Swapped tasks ${iOne+1} & ${iTwo+1}`)
